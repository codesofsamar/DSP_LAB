...
void pll(pll_state_def *s, const float x_in[], float y_out[])
{
    ...
    for (n=0; n<BUFFER_SAMPLES; n++)
    {
		...
		daccum = s->f0 - s->K/(2.0*M_PI)*v_n;
		s->accum = s->accum + daccum ;
	...
	if (s->mult == 2.0) {
			s->accum2 = s->accum2 + daccum * s->mult;
			s->accum2 = s->accum2 - floor(s->accum2);
			y_n2 = sin_table[(int)(((float)SIN_TABLE_SIZE * s->accum2))];
			y_out[n] = y_n2*0.2;
		}
		else if (s->mult == 0.5) {
			s->accum2 = s->accum2 + daccum;
			s->accum2 = s->accum2 - 2.0*floor(s->accum2/2.0);
			y_n2 = sin_table[(int)(((float)SIN_TABLE_SIZE* 0.5 * s->accum2))];
			y_out[n] = y_n2*0.2;
		}
        else {
	        y_out[n] = y_n*0.2;
